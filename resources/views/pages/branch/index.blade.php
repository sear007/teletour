		...
		<div class="row no-gutters">
			...
			<div class="col-lg-6">
				<div class="room-wrap d-md-flex ftco-animate">
					<a href="#" class="img {{ $key == 2 || $key == 3 ? 'order-md-last' : '' }}"><img src="{{$hotel->feature_image}}" alt="" height="100%" width="100%"></a>
					<div class="half left-arrow d-flex align-items-center">
						<div class="text p-4">
							...
							<h3 class="mb-3"><a href="{{route('hotel.show', [$hotel->id, slug($hotel->name)])}}">{{$hotel->name}}</a></h3>
							@foreach ($hotel->rooms as $room)
							<span class="badge badge-info">{{$room->name}}</span>
							@endforeach
							<p class="pt-1"><a href="{{route('hotel.show', [$hotel->id, slug($hotel->name)])}}" class="btn-custom px-3 py-2 rounded">View Details <span class="icon-long-arrow-right"></span></a></p>
						</div>
					</div>
				</div>
			</div>
